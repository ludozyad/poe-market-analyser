...
class SimpleExpectedValueProfitEngine:
    ...
    def calculate(
        ...
        price_book: ManualPriceBook | PriceBook,
        ...
        for ingredient in recipe.ingredients:
            if not ingredient.price_lookup:
                continue
            price = _get_unit_price(price_book, ingredient.id)
            if price is None:
                missing_prices.append(MissingPrice(ingredient.id, "No price provided."))
                continue
            ingredient_cost += ingredient.resolved_quantity * price
...
def _get_unit_price(price_book: ManualPriceBook | PriceBook, ingredient_id: str) -> float | None:
    if isinstance(price_book, PriceBook):
        return price_book.get_unit_price(ingredient_id)

    price = price_book.get_price(ingredient_id)
    if price is None:
        return None
