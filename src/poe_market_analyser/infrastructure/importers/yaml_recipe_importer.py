...
class YamlRecipeImporter:
    ...
    def _map_document(self, document: dict[str, Any]) -> CraftingRecipe:
        ...
        pricing = PricingConfig(
            ...
            output=_map_output_pricing(pricing_section.get("output")),
        )
...
def _map_output_pricing(value: Any) -> OutputPricingConfig | None:
    if value is None:
        return None
    if not isinstance(value, dict):
        raise YamlRecipeImportError("pricing.output must be a mapping/object when provided.")
    return OutputPricingConfig(
        mode=str(value.get("mode", "manual_estimate")),
        estimated_sale_price_chaos=_optional_float(value.get("estimated_sale_price_chaos")),
        failed_resale_value_chaos=float(value.get("failed_resale_value_chaos", 0.0)),
        confidence=str(value.get("confidence", "unknown")),
        source=str(value.get("source", "recipe")),
...
def _map_ingredient(document: dict[str, Any]) -> Ingredient:
    ...
    quantity = document.get("quantity", document.get("quantity_per_attempt", 1.0))
    expected_quantity = _optional_float(document.get("expected_quantity"))
    return Ingredient(
        id=str(document.get("id", "")),
        type=str(document.get("type", "")),
        price_lookup=bool(document.get("price_lookup", True)),
        quantity=float(quantity),
        quantity_mode=str(document.get("quantity_mode", "fixed")),
