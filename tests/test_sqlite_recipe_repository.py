...
def test_repository_stores_importable_raw_recipe_document(tmp_path):
    recipe = YamlRecipeImporter().import_file(SAMPLE_RECIPE)
    repository = SqliteRecipeRepository(tmp_path / "poe_market.db")

    repository.save(recipe)
    document = repository.get_document_json("poe1_mirage_viper_touch_spiked_gloves")
    loaded = YamlRecipeImporter().import_document(document)

    assert loaded.id == recipe.id
    assert loaded.ingredients[1].metadata["market_name"] == "Gilded Fossil"
